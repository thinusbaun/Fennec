...
void LogEntry::merge(const LogEntry& entry) {
  for (auto it = entry.getPartsMap().begin(); it != entry.getPartsMap().end();
       it++) {
    if (!mPartsMap.contains(it.key()) ||
        !mPartsMap[it.key()].canConvert(QVariant::String)) {
      mPartsMap[it.key()] = it.value();
    } else if (mPartsMap[it.key()].type() == QVariant::String) {
